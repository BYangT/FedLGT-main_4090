...
if __name__ == '__main__':
    ...
    if do_unlearn:
        ...
        if dataset == "voc":
            ...
            # VOC 里你之前一直用 14 = Person
            default_forget_cls = 14
            csv_prefix = "voc_full_fed_unlearn"

        elif dataset == "coco":
            # 这里给一个标准 COCO80 顺序的例子（你可以根据自己 annotations_pytorch 里的顺序调整）
            COCO_CLASSES = [
            ...
            # 默认忘却类你可以按自己需要改（比如 person=0），也可以用命令行 --forget_cls 覆盖
        ...
        save_voc_per_class_report_csv(  # 函数名虽然写 voc，只要内部按 logits/labels 写就能通用
        ...
        summary = summarize_forget_vs_others(
            ...
            forget_cls=forget_cls,
            class_names=CLASS_NAMES,  # 或 COCO80_CLASSES
        ...
        save_voc_per_class_report_csv(
            all_targs_before=all_targs_before,
            all_preds_before=all_preds_before,
            all_targs_after=all_targs_rec,  # 恢复后
        ...
        summary = summarize_forget_vs_others(
            ...
            forget_cls=forget_cls,
            class_names=CLASS_NAMES,  # 或 COCO80_CLASSES
            csv_path=os.path.join(args.results_new,
                                  f"new_recovery_forget_vs_others_cls{forget_cls}.csv"),
            threshold=0.5,
        )
